...
def init_db():
    with db() as con:
        ...
        # Trivsel
        con.executescript("""
            CREATE TABLE IF NOT EXISTS trivsel_runder (
                id        INTEGER PRIMARY KEY AUTOINCREMENT,
                år        INTEGER NOT NULL,
            ...
            CREATE TABLE IF NOT EXISTS trivsel_svar (
                ...
                spm1      INTEGER NOT NULL,
                spm2      INTEGER NOT NULL,
                tidspunkt TEXT NOT NULL
            );
...
def aktiv_runde() -> dict | None:
    with db() as con:
        r = con.execute(
            "SELECT * FROM trivsel_runder WHERE aktiv=1 ORDER BY id DESC LIMIT 1"
        ).fetchone()
    return dict(r) if r else None

def opprett_runde(år: int, måned: int) -> int:
    with db() as con:
        con.execute("UPDATE trivsel_runder SET aktiv=0")
        cur = con.execute(
            "INSERT INTO trivsel_runder (år, måned, opprettet, aktiv) VALUES (?,?,?,1)",
            (år, måned, datetime.now().isoformat())
        )
...
def registrer_trivsel_svar(runde_id: int, trivsel_token: str, spm1: int, spm2: int) -> bool:
    with db() as con:
        if con.execute(
            ...
            return False
        con.execute(
            "INSERT INTO trivsel_svar (runde_id, spm1, spm2, tidspunkt) VALUES (?,?,?,?)",
...
def hent_trivsel_resultater(runde_id: int) -> dict:
    with db() as con:
        rader = con.execute(
            "SELECT spm1, spm2 FROM trivsel_svar WHERE runde_id=?", (runde_id,)
        ).fetchall()
        antall_brukere = con.execute("SELECT COUNT(*) FROM brukere").fetchone()[0]
    if not rader:
        return {"antall_svar": 0, "antall_brukere": antall_brukere,
                "snitt_spm1": None, "snitt_spm2": None,
                "fordeling_spm1": [0]*7, "fordeling_spm2": [0]*7}
    v1 = [r["spm1"] for r in rader]
    v2 = [r["spm2"] for r in rader]
    return {
        "antall_svar": len(rader),
        "antall_brukere": antall_brukere,
        "snitt_spm1": round(sum(v1) / len(v1), 2),
        "snitt_spm2": round(sum(v2) / len(v2), 2),
        "fordeling_spm1": [v1.count(i) for i in range(1, 8)],
...
async def trivsel_send_inn(request: Request, trivsel_token: str):
    ...
    runde = aktiv_runde()
    if not runde:
        return HTMLResponse("<h1>Ingen aktiv runde</h1>", status_code=400)
    form = await request.form()
    try:
        spm1 = int(form.get("spm1", 0))
        spm2 = int(form.get("spm2", 0))
        if not (1 <= spm1 <= 7 and 1 <= spm2 <= 7):
            raise ValueError
    except (ValueError, TypeError):
...
async def admin_trivsel(request: Request):
    ...
    return render("admin_trivsel.html",
        ...
        måneds_navn=MÅNEDS_NAVN,
        melding=request.query_params.get("melding", ""),
        år_nå=date.today().year, måned_nå=date.today().month,
    )

@app.post("/admin/trivsel/ny-runde")
async def admin_ny_trivsel_runde(request: Request):
    if not er_innlogget(request):
        return RedirectResponse("/admin", status_code=303)
    form = await request.form()
    try:
        år = int(form.get("år", date.today().year))
        måned = int(form.get("måned", date.today().month))
        if not (1 <= måned <= 12):
            raise ValueError
    except (ValueError, TypeError):
        return RedirectResponse("/admin/trivsel?melding=Ugyldig+dato", status_code=303)
    opprett_runde(år, måned)
    return RedirectResponse("/admin/trivsel?melding=Ny+runde+opprettet", status_code=303)

@app.get("/api/trivsel/lenker")
async def api_trivsel_lenker(request: Request, key: Optional[str] = Query(default=None)):
    api_key_ok = EXPORT_API_KEY and key and secrets.compare_digest(key, EXPORT_API_KEY)
    if not api_key_ok:
        return JSONResponse({"error": "Ikke autorisert"}, status_code=403)
    runde = aktiv_runde()
